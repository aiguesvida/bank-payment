...
class TestAccountPaymentPurchase(TransactionCase):
    ...
    def _confirm_purchase(self, purchase):
        """Exercise the real signature gate when ViDA is installed, not a bypass."""
        ...
        payload = approval.get_signing_payload()
        signed = sign_pdf(base64.b64decode(payload["pdf"]), make_pki()["signer"])
        result = approval.submit_signature(
            base64.b64encode(signed).decode(), payload["token"]
        )
        self.assertEqual(approval.state, "signed")
        self.assertEqual(purchase.state, "purchase")
        return result

    @mute_logger("odoo.models.unlink")
    def test_purchase_order_create_invoice(self):
        self._confirm_purchase(self.purchase)
    ...
    def test_from_purchase_order_invoicing(self):
        ...
        self._confirm_purchase(purchase2)
